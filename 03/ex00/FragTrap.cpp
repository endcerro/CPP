...
/*   Updated: 2021/04/23 12:48:38 by edal--ce         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::cout << _name << " has entered the game by copy" << std::endl;
}
...
void FragTrap::takeDamage(unsigned int amt)
{
	...
	std::cout << _name << " takes a hit, causing " << damage << " dmg, hp is now ";
	_hp = (damage >= _hp) ? 0 : _hp - damage;
	std::cout << _hp << std::endl;
}
void FragTrap::beRepaired(unsigned int amt)
{
	std::cout << _name << " heals for " << amt << "hp, is now ";
