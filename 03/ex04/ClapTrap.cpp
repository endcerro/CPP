...
/*   Updated: 2021/04/23 15:11:55 by edal--ce         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(unsigned int hp, unsigned int ep, unsigned int level, std::string name,
 std::string type,	unsigned int cqc_d, unsigned int rng_d, unsigned int armr) : 
_hp(hp), _max_hp(hp), _ep(ep), _max_ep(ep), _lvl(level), _name(name), _type(type), _cqc_d(cqc_d), _rng_d(rng_d), _armr(armr)
{
	std::cout << "CL4P-TP " <<_name << " has entered the game" << std::endl;
}
